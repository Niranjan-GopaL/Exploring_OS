...
1. Biggest Problem : window's don't have fcntl ; needs win32 API calls ; Not Trivial
2. Due to the presence of Windows-style line endings (`\r\n`) in a script meant to be run on a Unix-like system (which uses `\n` line endings)
```sh
> zsh: ./client.py: bad interpreter: /bin/python3^M: no such file or directory
```
```sh
$ tr -d '\r' < server.py > server_fixed.py
$ mv server_fixed.py server.py
```
3. BUFFER SIZE too small ; Because TCP is persistant, whatever packets it didn't send will be sent in next sendall()